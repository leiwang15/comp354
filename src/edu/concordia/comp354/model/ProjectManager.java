...
//        System.out.println(userName);
    ...
    public void evaDateSelected(String dateStr) {
        int date = getDateFromStr(dateStr);

        if (date != UNDEFINED) {
            EarnedValuePoint point = currentProject.findEVAPoint(date);

            if (point != null) {
                EarnedValueAnalysis eva = new EarnedValueAnalysis(activityNetwork, date, point.getActualCost());

                evaRenderer.populateEVA(eva);
            }
        }
    }

    private int getDateFromStr(String dateStr) {
        return dateStr != null ? currentProject.date2day(LocalDate.parse(dateStr, EVATab.DATE_FORMAT)) : ProjectManager.UNDEFINED;
    }

    public EVARenderer getEvaRenderer() {
        return evaRenderer;
    }

    public void setEvaRenderer(EVARenderer evaRenderer) {
        this.evaRenderer = evaRenderer;
    }

    public void computeEVAforDate(String dateStr) {

        if (currentProject.generateEVA(activityNetwork, getDateFromStr(dateStr))) {
            evaDateSelected(dateStr);
            activityEntryRenderer.setActivityList();
        }
    }
